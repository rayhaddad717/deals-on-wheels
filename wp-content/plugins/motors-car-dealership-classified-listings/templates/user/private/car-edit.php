    ...
    <?php if (get_post_status($id) != 'pending'): ?>
        <div class="stm_edit_disable_car heading-font">
            <?php if (!empty($user_id)): ?>
                <a href="<?php echo esc_url(apply_filters( 'stm_get_add_page_url', 'edit', $id)); ?>">
                    <?php esc_html_e('Edit', 'stm_vehicles_listing'); ?>
                </a>
            <?php endif; ?>
            <?php if (get_post_status($id) == 'draft'): ?>
            <?php else: ?>
                <a href="<?php echo esc_url(add_query_arg(array('stm_disable_user_car' => $id), apply_filters( 'stm_get_author_link',''))); ?>"
                   class="disable_list"
    ...
    <?php else: ?>
        <div class="stm_edit_pending_car">
            ...
            <a class="stm-delete-confirmation"
               href="<?php echo esc_url(add_query_arg(array('stm_move_trash_car' => $id), apply_filters( 'stm_get_author_link',''))); ?>"
