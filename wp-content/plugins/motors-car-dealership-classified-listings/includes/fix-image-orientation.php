...
function stm_transfer_iptc_exif_to_image( $image_info, $destination_image, $original_orientation ) {
	...
	$destination_image_contents = wp_remote_get( $destination_image );
	...
	while ( ( ( ord( $destination_image_contents[0] ) << 8 | ord( $destination_image_contents[1] ) ) & 0xFFF0 ) === 0xFFE0 ) {
		$segment_length      = ( ( ord( $destination_image_contents[2] ) << 8 | ord( $destination_image_contents[3] ) ) & 0xFFFF );
		$iptc_segment_number = ( ord( $destination_image_contents[1] ) & 0x0F );   // Last 4 bits of second byte is IPTC segment #
		if ( $segment_length <= 2 ) {
			return false;
		}
	...
	$output_file = fopen( $destination_image, 'w' ); //phpcs:ignore
	if ( $output_file ) {
		return fwrite( $output_file, $portion_to_add . $destination_image_contents ); //phpcs:ignore
	}
